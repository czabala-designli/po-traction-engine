...
**Finding 2b — Go-to-market order never made it into the saved files**
The Go-to-market order paragraph from the Phase 1 bridging question existed in the conversation output but was never written to the saved .md files — it had to be added manually to icp-communities-club-director.md as Section 0. Fixed by adding Section 0 to the Phase 3 output template, rendered only when two ICPs are confirmed. Same text written into both ICP files (redundancy without added complexity — a PO opening either file gets the full sequencing picture without cross-referencing the other doc).

**Finding 2c — single-ICP file had no slug in the name**
Original convention: single ICP saved to `docs/icp-communities.md` (no slug). Problem: running the skill twice on the same project would overwrite the first file; also inconsistent with the two-ICP convention. Fixed: always use `docs/icp-communities-[archetype-slug].md` regardless of ICP count. Slug = archetype label lowercased with hyphens (e.g. `icp-communities-volleyball-coach.md`). One file per ICP, always.
